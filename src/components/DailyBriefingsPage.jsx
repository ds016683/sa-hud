    ...
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '24px 16px', fontFamily: 'Arial, Helvetica, sans-serif' }}>
      ...
      {tab === 'archive' && (
        <div>
          ...
          {[yesterday].map(function(dateStr) {
            var label = new Date(dateStr + 'T12:00:00').toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' })
            return (
              <div key={dateStr} style={{ border: '1px solid #E5E7EB', borderRadius: 10, overflow: 'hidden', marginBottom: 12 }}>
                ...
                <div style={{ padding: 16 }}>
                  <DayView dateStr={dateStr} dateLabel={label} />
                </div>
              </div>
            )
          })}
