  ...
  return (
    ...
    >
      <div className="p-3 space-y-2.5">
        ...
        <div className="flex flex-wrap gap-1 items-center">
          ...
          {addingTag ? (
            ...
            <button
              ...
              className="flex items-center gap-0.5 px-1.5 py-0.5 text-[10px] text-gray-400 hover:text-violet-600 transition-colors border border-dashed border-gray-200 hover:border-violet-300 rounded-full"
            >
              <Tag size={8} /> tag
            </button>
          )}
        ...
        {/* Next action */}
        <div>
          <label className="text-[10px] text-gray-400 uppercase tracking-wider font-mono">Next action</label>
          <input
            ...
            className="w-full mt-0.5 text-xs text-gray-700 border border-gray-100 rounded px-2 py-1 focus:outline-none focus:border-violet-300 bg-gray-50 placeholder:text-gray-300"
          />
        </div>
        ...
        <div>
          <button
            ...
            className="flex items-center gap-1 text-[10px] text-gray-400 hover:text-violet-600 transition-colors uppercase tracking-wider font-mono"
          >
          ...
          {expanded && (
            <textarea
              ...
              rows={3}
              className="w-full mt-1 text-xs text-gray-700 border border-gray-200 rounded px-2 py-1.5 focus:outline-none focus:border-violet-300 resize-none bg-gray-50 placeholder:text-gray-300"
            />
          )}
        </div>
        ...
        {/* Dropbox path */}
        <div>
          <label className="text-[10px] text-gray-400 uppercase tracking-wider font-mono">Dropbox path</label>
          <input
            ...
            className="w-full mt-0.5 text-xs text-gray-500 border border-gray-100 rounded px-2 py-1 focus:outline-none focus:border-violet-300 bg-gray-50 placeholder:text-gray-300 font-mono"
          />
        </div>
        ...
        {isDisposition && (
          <div>
            <label className="text-[10px] text-gray-500 uppercase tracking-wider font-mono mb-1 block">Disposition</label>
        ...
        <div className="flex gap-3 pt-0.5">
          <span className="text-[9px] text-gray-300 font-mono">Created {formatDateShort(idea.created_at)}</span>
          {idea.last_modified !== idea.created_at && (
            <span className="text-[9px] text-gray-300 font-mono">Modified {formatDateShort(idea.last_modified)}</span>
          )}
        </div>
    ...
    <div ref={setNodeRef} style={style}>
      <div className="relative">
        ...
        <div
          {...attributes}
          {...listeners}
          className="absolute top-2 right-8 w-5 h-5 flex items-center justify-center text-gray-300 hover:text-violet-400 cursor-grab active:cursor-grabbing z-10 opacity-0 group-hover:opacity-100 transition-opacity"
  ...
  return (
    <div className="flex flex-col min-w-0" style={{ minWidth: 260, maxWidth: 320, flex: '1 1 260px' }}>
      ...
      <div className="mb-3">
        ...
        <p className="text-[10px] text-gray-400 font-mono leading-relaxed px-0.5">{meta.description}</p>
      </div>
        ...
        <div className="flex flex-col gap-2 flex-1">
          {stageIdeas.length === 0 ? (
            <div className="border border-dashed border-gray-200 rounded-lg p-4 text-center">
              <p className="text-[11px] text-gray-400 font-mono italic">{meta.encouragement}</p>
            </div>
          ) : (
            stageIdeas.map(idea => (
              <SortableIdeaCard
    ...
    <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-5 space-y-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            <label className="text-xs text-gray-500 font-mono uppercase tracking-wider block mb-1">Title</label>
            <input
              autoFocus
              value={title}
          ...
          <div>
            <label className="text-xs text-gray-500 font-mono uppercase tracking-wider block mb-1">Tags <span className="normal-case text-gray-400">(comma-separated, optional)</span></label>
            <input
              value={tagsInput}
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 text-sm text-gray-500 rounded-lg border border-gray-200 hover:bg-gray-50 transition-colors"
            >
              Cancel
            </button>
          </div>
    ...
    <div className="min-h-screen px-4 py-6" style={{ background: '#F8F7FF' }}>
      ...
      <div className="max-w-[1400px] mx-auto">
        ...
        <div className="flex flex-wrap gap-2 mb-6 items-center">
          <span className="text-[10px] text-gray-400 font-mono uppercase tracking-wider mr-1">Filter:</span>
          ...
          <div className="flex gap-1">
            <button
              ...
              className={`px-2.5 py-1 text-[10px] font-mono rounded border transition-colors ${
                !filterStage ? 'bg-violet-50 text-violet-700 border-violet-200' : 'text-gray-400 border-gray-200 hover:border-violet-200'
              }`}
            >
              All stages
            </button>
            {STAGES.map(s => (
              <button
                ...
                className={`px-2.5 py-1 text-[10px] font-mono rounded border transition-colors ${
                  filterStage === s ? 'bg-violet-50 text-violet-700 border-violet-200' : 'text-gray-400 border-gray-200 hover:border-violet-200'
                }`}
              >
          ...
          {allTags.length > 0 && (
            <div className="flex gap-1 items-center ml-2 pl-2 border-l border-gray-200">
              <button
                ...
                className={`px-2.5 py-1 text-[10px] font-mono rounded border transition-colors ${
                  !filterTag ? 'bg-violet-50 text-violet-700 border-violet-200' : 'text-gray-400 border-gray-200 hover:border-violet-200'
                }`}
              >
                All tags
              </button>
              {allTags.map(tag => (
                <button
                  ...
                  className={`px-2.5 py-1 text-[10px] font-mono rounded border transition-colors ${
                    filterTag === tag ? 'bg-violet-50 text-violet-700 border-violet-200' : 'text-gray-400 border-gray-200 hover:border-violet-200'
                  }`}
                >
                  #{tag}
                </button>
        ...
        {ideas.length === 0 && (
          <div className="text-center py-20 space-y-3">
            ...
            <p className="text-gray-500 font-mono text-sm">No ideas in the pipeline yet.</p>
            <p className="text-gray-400 text-xs">Hit "New Idea" to plant the first seed.</p>
          </div>
        )}
      </div>
