...
const PortfolioPage = () => {
  ...
  if (loading) {
    return (
      <div className="max-w-5xl mx-auto px-4 py-6 text-center text-game-text-muted text-sm font-mono">
        Loading portfolio...
      </div>
    )
  }
    ...
    <div className="max-w-5xl mx-auto px-4 py-6 space-y-8">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={exportToMarkdown} className="flex items-center gap-2 px-3 py-2 rounded border border-game-border text-game-text-dim text-xs font-mono hover:text-game-gold hover:border-game-gold/30 transition-colors">
            <Download size={14} /> Export
          </button>
          <button onClick={() => setShowCreateModal(true)} className="flex items-center gap-2 px-3 py-2 rounded bg-game-gold/20 border border-game-gold/40 text-game-gold text-xs font-mono hover:bg-game-gold/30 transition-colors">
      ...
      <section className="rounded-xl border-2 border-red-300 bg-red-50 px-5 py-4">
        <button onClick={() => toggleSection('q2-must')} className="w-full flex items-center gap-3 mb-3 group text-left">
          <Target size={16} className="text-red-600 shrink-0" />
          <h2 className="font-game text-sm text-red-700 uppercase tracking-[0.15em] flex-1">What Must Be True by June 30th</h2>
          <span className="text-xs font-mono text-red-500">{q2Projects.length} commitment{q2Projects.length !== 1 ? 's' : ''}</span>
        ...
        {!q2Collapsed && (
          q2Projects.length === 0 ? (
            <div className="text-center py-4">
              ...
              <button onClick={() => setShowCreateModal(true)} className="mt-2 text-xs font-mono text-red-600 hover:underline">+ Add a commitment</button>
            </div>
          ) : (
      ...
      {SECTIONS.map(section => {
        ...
        return (
          <section key={section.id}>
            <button onClick={() => toggleSection(section.id)} className="w-full flex items-center gap-3 mb-3 group text-left">
              <span className={`w-2 h-2 rounded-full ${section.color} shrink-0`} />
              <h2 className={`font-game text-sm ${section.accent} uppercase tracking-[0.15em] flex-1`}>{section.label}</h2>
              <span className="text-xs font-mono text-game-text-dim">{projects.length} project{projects.length !== 1 ? 's' : ''}</span>
              {isCollapsed ? <ChevronRight size={14} className="text-game-text-dim group-hover:text-game-text-muted" /> : <ChevronDown size={14} className="text-game-text-dim group-hover:text-game-text-muted" />}
            </button>
            {!isCollapsed && (
              projects.length === 0 ? (
                <div className={`rounded-lg border border-dashed ${section.border} ${section.bg} px-4 py-6 text-center`}>
                  ...
                  <button onClick={() => setShowCreateModal(true)} className={`mt-2 text-xs font-mono ${section.accent} hover:underline transition-colors`}>+ Add one</button>
                </div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      {archive.length > 0 && (
        <section>
          <button onClick={() => setArchiveOpen(!archiveOpen)} className="font-game text-xs text-game-text-dim uppercase tracking-[0.2em] mb-3 flex items-center gap-2 hover:text-game-text-muted transition-colors">
            {archiveOpen ? <ChevronDown size={12} /> : <ChevronRight size={12} />}
            Completed Archive <span>({archive.length})</span>
          </button>
          {archiveOpen && (
                  ...
                  <div className="bg-game-panel/20 border border-game-border/30 rounded-lg overflow-hidden">
                    {projects.map(project => (
                      <div key={project.id} className="flex items-center gap-3 px-3 py-2 border-b border-game-border/20 last:border-0 opacity-50">
                        ...
                        {project.category && <span className="text-[10px] px-2 py-0.5 rounded border border-game-border/30 text-game-text-dim font-mono">{project.category}</span>}
